# Flagline App Store ship checklist

This is Jon's Mac checklist for the Monday iOS Safari release. It is not an automated code path.

## Build and sign

- [ ] Pull the reviewed Flagline source onto a Mac with full Xcode.
- [ ] Run `./safari/convert.sh` with the default app name `Flagline` and bundle ID `com.aevum.flagline`.
...
- [ ] Keep the containing app a thin shell: Flagline title, three enablement steps, and a link to Safari settings if the current iOS API permits it.
- [ ] Do not add accounts, analytics, a backend, subscriptions, or in-app purchase to v1.

The three containing-app steps:

1. Open Settings → Safari → Extensions and enable Flagline.
2. Open Safari → x.com, not the X app.
3. Allow Flagline for x.com; flags appear and a flag tap hides that country.
...
- [ ] Enable Flagline on a physical iPhone or iPad.
- [ ] Allow Flagline for x.com.
...
- [ ] Inspect network traffic and confirm there is no Flagline backend request.
...
- [ ] Add the display name **Flagline** and subtitle **Location flags and country filter for X**.
